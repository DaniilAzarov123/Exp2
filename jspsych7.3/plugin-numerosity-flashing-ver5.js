            ...
            if (trial.render_on_canvas) {
              // console.log("if");
                ...
                /* var animate_interval = setInterval(() => {
                    var showImage = true;
                    animate_frame++;
                    if (animate_frame == trial.stimulus.length) {
                        animate_frame = 0;
                        reps++;
                        if (reps >= trial.sequence_reps) {
                            // end_trial();                                // changed the spelling
                ...
                const show_next_frame = () => {
                    if (trial.render_on_canvas) {
                        // display_element.querySelector("#jspsych-image-slider-response-stimulus").classList.add("visible") // style.visibility = "visible";
                        
                        var map;
                        // var spriteSize;
                        var sprites;
                        var img = new Image();
                        var sprite_ind_in_list = sprite_list.findIndex(a => a == trial.stimulus[animate_frame]); // change 0 to animate_frame
                        ...
                        img.onload = () => {
                            ...
                            if (!image_drawn) {
                                ...
                                sprites_list.push(sprites);
                                // trial.sprites.push(sprites);
                                getHeightWidth(); // only possible to get width/height after image loads
                                // let positions = [];
                                const rng = new SeedableRNG(trial.random_seed); // Initialize with a seed (like set.seed(1234) in R)
                                for (let i = 0; i < trial.numerosity; i++) {
                                    // let placed = false;
                                    //var rng = new SeedableRNG(trial.random_seed); // Initialize with a seed (like set.seed(1234) in R)
                                    // while (!placed) {
                                    let sprite = Math.floor(rng.random() * map.length); // random sprite (e.g. a butterfly) from a spritesheet (all butterflies)
                                    let x_loc = positions[i].x;
                                    let y_loc = positions[i].y;
                                    ...
                                    ctx.drawImage(img,                                                      // image source
                                        map[sprite][0] * spriteSize, map[sprite][1] * spriteSize,           // x, y coordinates of the image we want to crop (crop a butterfly from the butterflies sheet)
                        ...
                        img.src = shownImage; // which image we are showing at the moment
                    }
                    current_stim = trial.stimulus[animate_frame];
                    ...
                    if (trial.frame_isi > 0) {
                        this.jsPsych.pluginAPI.setTimeout(() => {
                            display_element.querySelector("#jspsych-image-slider-response-stimulus").style.visibility = "hidden";
                            /* let imgElement = display_element.querySelector("img");
                            console.log("img element: ", imgElement);
                            if (imgElement) {
                                display_element.removeChild(imgElement);
                            } */
                ...
                console.log("img.src:",shownImage);
                console.log("stim:",trial.stimulus); */
