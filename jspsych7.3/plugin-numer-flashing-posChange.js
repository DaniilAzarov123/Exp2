var jsPsychNumerosityEstimationFlashingPositions = (function (jspsych) {
    'use strict';
  
    const info = {
        name: "numerosity-estimation-flashing-positions",
        parameters: {
    ...
    class ImageSliderResponsePlugin {
        ...
        trial(display_element, trial) {
            ...
            if (trial.render_on_canvas){
                ...
                function set_positions(seed) {
                    let rng_i = new SeedableRNG(seed);
                    let positions = [];
                    // let attempts = 0;
                    while (positions.length < trial.numerosity) {
                        // attempts++;
                        let x_loc = Math.floor(rng_i.random() * (canvas.width - spriteSize));
                        let y_loc = Math.floor(rng_i.random() * (canvas.height - spriteSize));
                    ...
                    /* if (attempts >= 1000) {
                        console.warn("Too many attempts to place items without overlap.");
                    } */
                    return positions;
                }
                let positions = set_positions(trial.random_seed);
                ...
                // (if it's static condition - it will last for the whole trial as
                // stated in the design matrix)
                function animateSprites() {
                    setInterval(() => {
                        ctx.clearRect(0, 0, canvas.width, canvas.height);
                        currentSpriteIndex = (currentSpriteIndex + 1) % spriteSheets.length;
                        // let rng_i = new SeedableRNG(trial.random_seed + currentSpriteIndex);

                        if (trial.pos_change){
                            positions = set_positions(trial.random_seed + currentSpriteIndex);
                        }
                ...
                img.onload = () => {
                    ...
                    setTimeout(() => {
                        drawSprites();
                        animateSprites();
                    }, trial.fix_cross_dur); // Wait 1000ms (showing fixation cross)
                };
                drawFixationCross();
            ...
            const end_trial = () => {
                ...
                /* // Clear the display
                display_element.innerHTML = '';  // this removes stimulus and shows a blank screen

                // Wait 1000 ms, then save data and finish the trial
                this.jsPsych.pluginAPI.setTimeout(() => {
                    const trialdata = {
                    rt: response.rt,
                    stimulus: trial.stimulus,
                    image: spriteSheets.map(sheet => sheet.image),
                    response: response.response,
                    absDeviation: Math.abs(response.response - trial.numerosity)
                    };
